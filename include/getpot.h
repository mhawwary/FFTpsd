/**  Updates
 * 10-24-1014 Lei Shi Found a bug, e.g. u=5 and u = 5 are ok, 
 *                    but parser can not find the correct value for u= 5
 * 09-3-2012  Lei Shi 
 * 12-1-2012  Lei Shi Change the default behavior of operator()
 * 11-18-2012 Lei Shi Add support for parsing vector<string> as an input
 *    inline GetPot(std::vector<std::string> vec_string,
 *                 const char* FieldSeparator=0x0);
 *    First item in vector<string> is dumb. So the size of input vector<string>   
...
public:
    ...
    // (*) variables -----------------------------------------------------------
    //     -- scalar values
    inline int                operator()(const char* VarName, int           Default, bool NoDefault=true) const;
    inline double             operator()(const char* VarName, const double& Default, bool NoDefault=true) const;
    inline const std::string  operator()(const char* VarName, const char*   Default, bool NoDefault=true) const;
    //     -- vectors
    inline int                operator()(const char* VarName, int Default, unsigned Idx, bool NoDefault=true) const;
    inline double             operator()(const char* VarName, const double& Default, unsigned Idx, bool NoDefault=true) const;
    inline const std::string  operator()(const char* VarName, const char* Default, unsigned Idx, bool NoDefault=true) const;
    ...
    //return a string list of sub-sections
    inline std::vector<std::string> sub_sections(const char* prefix);
...
inline
GetPot::GetPot(std::vector<std::string> vec_string,
               const char* FieldSeparator /* =0x0 */)     
    // leave 'char**' non-const to honor less capable compilers ... 
{
  // TODO: Ponder over the problem when the argument list is of size = 0.
  ...
  //    which facilitates error checking in many functions. Also the user will be able to
  //    retrieve the name of his application by "get[0]"
  _apriori_argv.push_back(vec_string[0]);
  unsigned int i=1;
  for(; i<vec_string.size(); ++i) {
    std::string tmp(vec_string[i]);   // recall the problem with temporaries,
    _apriori_argv.push_back(tmp);       // reference counting in arguement lists ...
  }
  __parse_argument_vector(_apriori_argv);
}
...
GetPot::operator=(const GetPot& That)
{
    ...
    section_list    = That.section_list;
    _field_separator = That._field_separator;
    */
    __request_recording_f = That.__request_recording_f;
...
inline std::vector<std::string> GetPot::sub_sections(const char* prefix){
  std::vector<std::string> sub_section;

  int l=strlen(prefix)+1; //1 is for the /
  for(auto &str:section_list){
    std::size_t found = str.find(prefix);
    
    //??? if (found!=std::string::npos && found+l!=str.length())
    if (found==0 && found+l!=str.length())
      sub_section.push_back(str.substr(found+l));
...
//
inline int
GetPot::operator()(const char* VarName, int Default, bool NoDefault) const
{
    // (*) recording of requested variables happens in '__find_variable()'
    const variable*  sv = __find_variable(VarName);
    if( (sv==0 || (sv && sv->value.size()==0)) && NoDefault==false) 
      return Default;
    else if ((sv==0 || (sv && sv->value.size()==0)) &&NoDefault==true){
      std::stringstream ss;
      ss<<"Can't find "<<VarName<<" in config file"<<std::endl;
      std::cerr<<std::endl;
      std::cerr<<set_color(RED)<<"[  Failed in parsing input file  ] "<<set_color()<<ss.str();
      std::cerr<<std::endl;
...
inline double
GetPot::operator()(const char* VarName, const double& Default, bool NoDefault) const
{
    // (*) recording of requested variables happens in '__find_variable()'
    const variable*  sv = __find_variable(VarName);
...
inline const std::string
GetPot::operator()(const char* VarName, const char* Default, bool NoDefault) const
{
    // (*) recording of requested variables happens in '__find_variable()'
    const variable*  sv = __find_variable(VarName);
...
inline int
GetPot::operator()(const char* VarName, int Default, unsigned Idx, bool NoDefault) const
{
    // (*) recording of requested variables happens in '__find_variable()'
    const variable* sv = __find_variable(VarName);
    ...
    if( element == 0 && NoDefault==false)
      return Default;
    else if (element==0 &&NoDefault==true){
      std::stringstream ss;
      ss<<"Can't find "<<VarName<<" in config file"<<std::endl;
...
inline double
GetPot::operator()(const char* VarName, const double& Default, unsigned Idx, bool NoDefault) const
{
    // (*) recording of requested variables happens in '__find_variable()'
    const variable* sv = __find_variable(VarName);
...
inline const std::string
GetPot::operator()(const char* VarName, const char* Default, unsigned Idx, bool NoDefault) const
{
    // (*) recording of requested variables happens in '__find_variable()'
    const variable*  sv = __find_variable(VarName);
